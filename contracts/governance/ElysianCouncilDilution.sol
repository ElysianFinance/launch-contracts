...
/**
@title A contract that allows for the dilution of Elysian Council voting weights
*/

contract CouncilDilution is OwnedwManager {
    ...
    // @notice The length of a proposal (ECCP/EIP) voting period
    uint public proposalPeriod;

    /* Global variables */

    // @notice The ipfs hash of the latest Elysian Council election proposal
    string public latestElectionHash;

    struct ElectionLog {
        // @notice The ipfs hash of a particular Elysian Council election proposal
        string electionHash;
        // @notice A mapping of the votes allocated to each of the Elysian Council members
    ...
    struct ProposalLog {
        // @notice The ipfs hash of a particular ECCP/EIP proposal
        string proposalHash;
    ...
    uint256 public numElectionLogs;



    // @notice Given a voter address and a council member address, return the delegated vote weight for the most recent Elysian Council election
    mapping(address => mapping(address => uint)) public latestDelegatedVoteWeight;

    // @notice Given a council member address, return the total delegated vote weight for the most recent Elysian Council election
    ...
    // @notice Given a proposal hash (ECCP/EIP), return the ProposalLog struct associated
    ...
    constructor(address _owner, uint _numOfSeats) public OwnedwManager(_owner, _owner) {
    ...
    /**
    @notice A function to create a new ElectionLog, this is called to record the result of a Elysian Council election
    @param electionHash The ipfs hash of the Elysian Council election proposal to log
    @param nominatedCouncilMembers The array of the successful Elysian Council nominees addresses, must be the same length as the numOfSeats
    ...
    function logElection(
        ...
        ElectionLog storage newElectionLog = createElectionLog(electionHash, timestamp);
    ...
    function createElectionLog (string memory electionHash, uint timestamp) internal returns (ElectionLog storage newElectionLog) {

        ElectionLog storage newElectionLog = electionHashToLog[electionHash];
        newElectionLog.electionHash = electionHash;
        newElectionLog.created = timestamp;
        
        return newElectionLog;
    }

    /**
    @notice A function to created a new ProposalLog, this is called to record ECCP/EIP created and allow for dilution to occur per proposal.
    ...
    function dilute(string memory proposalHash, address memberToDilute) public {
        ...
        require(block.timestamp < proposalHashToLog[proposalHash].end, "dilution can only occur within the proposal voting period");
        ...
        if (proposalHashToMemberDilution[proposalHash][memberToDilute].exist) {
            ...
            DilutionReceipt storage newDilutionReceipt = createDilutionReceipt(proposalHash, memberToDilute, 0, dilutors, true);


            uint originalTotalDilutionValue = newDilutionReceipt.totalDilutionValue;

            newDilutionReceipt.dilutors.push(msg.sender);

            newDilutionReceipt.voterDilutions[
                msg.sender
            ] = latestDelegatedVoteWeight[msg.sender][memberToDilute];

            newDilutionReceipt.totalDilutionValue = latestDelegatedVoteWeight[
                msg.sender
            ][memberToDilute];
    ...
    function createDilutionReceipt (
        string memory proposalHash, 
        address memberToDilute,
        uint totalDilutionValue,
        address[] memory dilutors,
        bool exist
        ) internal returns (DilutionReceipt storage newDilutionReceipt) {

        DilutionReceipt storage newDilutionReceipt = proposalHashToMemberDilution[proposalHash][memberToDilute];
        newDilutionReceipt.proposalHash = proposalHash;
        newDilutionReceipt.memberDiluted = memberToDilute;
        newDilutionReceipt.totalDilutionValue = totalDilutionValue;
        newDilutionReceipt.dilutors = dilutors;
        newDilutionReceipt.exist = exist;

        return newDilutionReceipt;
    }
    ...
    function invalidateDilution(string memory proposalHash, address memberToUndilute) public {
        ...
        require(block.timestamp < proposalHashToLog[proposalHash].end, "undo dilution can only occur within the proposal voting period");
    ...
    /**
    @notice A function that can only be called by the OWNER that changes the number of seats on the Elysian Council
