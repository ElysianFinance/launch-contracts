...
*/
contract ElysianCouncil is OwnedwManager {
    // Event that is emitted when a new ElysianCouncil token is minted
    event Mint(uint256 indexed tokenId, address to);
    // Event that is emitted when an existing ElysianCouncil token is burned
    event Burn(uint256 indexed tokenId);
    // Event that is emitted when an existing ElysianCouncil token is Transferred
    event Transfer(address indexed from, address indexed to, uint256 indexed tokenId);
    // Event that is emitted when an existing ElysianCouncil token's uri is altered
     ...
     */
    constructor(string memory _name, string memory _symbol, address _owner) OwnedwManager(_owner, _owner) public {
        name = _name;
        symbol = _symbol;
    }
