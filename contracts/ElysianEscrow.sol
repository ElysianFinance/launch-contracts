...
contract ElysianEscrow is OwnedwManager , LimitedSetup(16 weeks), IHasBalance {
    ...
    /* An account's total vested elysian balance to save recomputing this for fee extraction purposes. */
    mapping(address => uint) public totalVestedAccountBalance;

    /* The total remaining vested balance, for verifying the actual elysian balance of this contract against. */
    ...
    constructor(address _owner, IElysian _elysian) public OwnedwManager(_owner, _owner) {
        elysian = _elysian;
    }

    /* ========== SETTERS ========== */

    function setElysian(IElysian _elysian) external onlyOwner {
        elysian = _elysian;
        emit ElysianUpdated(address(_elysian));
    }
     ...
     * in this contract, or a corresponding call to elysian.endow(), to ensure that when
     ...
     * @param quantity The quantity of LYS that will vest.
     */
    function appendVestingEntry(
        ...
        require(
            totalVestedBalance <= IERC20(address(elysian)).balanceOf(address(this)),
    ...
    /**
     * @notice Construct a vesting schedule to release a quantities of LYS
