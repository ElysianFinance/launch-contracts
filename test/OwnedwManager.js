...
contract('OwnedwManager', accounts => {
	...
	it('should revert when owner parameter is passed the zero address', async () => {
		await truffleAssert.reverts(
			Owned.new(ZERO_ADDRESS, ZERO_ADDRESS, {
	...
	it('should set owner address on deployment', async () => {
		const ownedContractInstance = await Owned.new(account1, account1, {
			from: deployerAccount
		});
	...
	it('should not nominate new owner when not invoked by current contract owner', async () => {
		...
		let ownedContractInstance = await Owned.new(account1, account1, {
			from: account2
		});
		await truffleAssert.reverts(
			ownedContractInstance.nominateNewOwner(nominatedOwner, {
				from: account2
			}),
	...
	it('should nominate new owner when invoked by current contract owner', async () => {
		...
		const txn = await ownedContractInstance.nominateNewOwner(nominatedOwner, {
			from: account1
		});
